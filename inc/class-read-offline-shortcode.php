...
class Read_Offline_Shortcode extends Read_Offline {
	...
	private function __construct() {
		...
		add_shortcode( 'pdf', [ $this, 'pdf' ] );
		add_shortcode( 'epub', [ $this, 'epub' ] );
		add_shortcode( 'mobi', [ $this, 'mobi' ] );
		add_shortcode( 'print', [ $this, 'prnt' ] );
		add_action( 'wp_enqueue_scripts', [ $this, 'shortcode_script_style' ] );
	}
	...
	private function _create_output( $type, $attributes, $post ) {
		...
		$readoffline = sprintf(
			'<div class="readoffline-shortcode"><a class="%s" href="%s" title="%s %s.%s" %s>%s</a></div>%s',
	...
	function shortcode_script_style() {
		wp_enqueue_style( 'read-offline-shortcode', READOFFLINE_URL . '/css/read-offline-shortcode.css', [], READOFFLINE_VERSION );
	}
}
