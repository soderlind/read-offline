...
class Read_Offline_UX extends Read_Offline {
	...
	private function __construct() {
		...
		add_action( 'the_content', [ $this, 'embed_post' ] );
		// add_action('the_category', array($this,'embed_category'));
		add_filter( 'get_the_archive_title', [ $this, 'embed_category' ] );
		add_action( 'wp_enqueue_scripts', [ $this, 'add_script_style' ] );
	}

	function add_script_style() {
		wp_enqueue_style( 'read-offline-embed', READOFFLINE_URL . '/css/read-offline-embed.css', [], READOFFLINE_VERSION );
		wp_enqueue_style( 'read-offline-css', READOFFLINE_URL . '/css/read-offline.css', [], READOFFLINE_VERSION );
		wp_enqueue_script( 'read-offline', READOFFLINE_URL . '/js/read-offline-wp.js', [ 'jquery' ], READOFFLINE_VERSION );

		// google analytics track event
		if ( '1' == parent::$options['misc']['google'] ) {
			wp_enqueue_script( 'read-offline-ga', READOFFLINE_URL . '/js/read-offline-ga.js', [ 'jquery' ], READOFFLINE_VERSION );
		}
	...
	function embed_post( $content ) {
		...
		foreach ( $formats as $type => $document_type ) {
			...
			$readoffline .= sprintf(
				'<div><a class="%s" href="%s" title="%s %s.%s" %s>%s</a></div>%s',
				$type,
