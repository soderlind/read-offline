...
class Read_Offline_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			'Read Offline', // Name
			[ 'description' => __( 'Adds a download link for the current post and page. Configurable in Settings->Read Offline', $this->localizationDomain ) ] // Args
		);
	}
	function widget( $args, $instance ) {
		...
		if ( $icononly === true ) {
			foreach ( $formats as $type => $document_type ) {
				printf(
					'<div><a class="%s" href="%s" title="%s %s.%s"></a></div>',
					$type,
					$this->ps_read_offline_url( $post->ID, $post->post_name, $type ),
					__( 'Download ', $this->localizationDomain ),
					$post->post_name,
					$type
				);
			}
			...
			foreach ( $formats as $type => $document_type ) {
				printf(
					'<li><a class="%s" href="%s" title="%s %s.%s">%s%s</a></li>',
					$type,
					$this->ps_read_offline_url( $post->ID, $post->post_name, $type ),
					__( 'Download ', $this->localizationDomain ),
					$post->post_name,
	...
	function form( $instance ) {
		...
		printf(
			...
			checked( $instance['icononly'], 'yes', false ),
			$this->get_field_id( 'icononly' ),
			$this->get_field_name( 'icononly' ),
			$this->get_field_id( 'icononly' ),
			__( 'Yes' ),
			checked( $instance['icononly'], 'no', false ),
			$this->get_field_id( 'icononly' ),
			$this->get_field_name( 'icononly' ),
			$this->get_field_id( 'icononly' ),
			__( 'No' )
