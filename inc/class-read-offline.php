...
class Read_Offline {
	...
	private function __construct() {
		...
		if ( is_admin() ) {
			add_action( 'admin_init', [ $this, 'read_offline_update' ] );
		}
		add_filter( 'upload_mimes', [ $this, 'add_epub_mobi_pdf_mime_types' ], 1, 1 );
		if ( is_multisite() ) {
		...
		if ( '1' == self::$options['misc']['cache'] ) {
			foreach ( $post_types as $post_type ) {
				add_action( 'save_post_' . $post_type, [ $this, 'save_as_attachment_to_post_type' ], 10, 2 );
			}
		}
	}
	...
	public static function query_url( $post_id, $name, $format, $refresh = false ) {
		...
		if ( get_option( 'permalink_structure' ) ) {
			...
			return sprintf(
				'%s/index.php?read_offline_id=%s&read_offline_name=%s&&read_offline_format=%s%s',
				site_url(),
				$post_id,
				$name,
				$format,
	...
	private function _create_tmp_directories() {
		...
		$directories = [ 'cache/read-offline/tmp', 'cache/read-offline/font' ];
	...
	private function _remove_tmp_directories() {
		...
		$directories = [ WP_CONTENT_DIR . '/cache/read-offline', WP_CONTENT_DIR . '/cache/read-offline-tmp' ];
		foreach ( $directories as $directory ) {
