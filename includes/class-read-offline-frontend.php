...
class Read_Offline_Frontend {
	public static function init() {
		add_filter( 'the_content', [ __CLASS__, 'append_save_as' ] );
		add_shortcode( 'read_offline', [ __CLASS__, 'shortcode' ] );
		add_action( 'wp_enqueue_scripts', [ __CLASS__, 'enqueue_assets' ] );
	}

	protected static function enabled_formats() {
		$settings = get_option( 'read_offline_settings_general', [] );
		$formats  = isset( $settings[ 'formats' ] ) ? (array) $settings[ 'formats' ] : [ 'pdf', 'epub' ];
		$formats  = apply_filters( 'read_offline_enabled_formats', $formats );
		return array_values( array_intersect( [ 'pdf', 'epub' ], $formats ) );
	}
	...
	protected static function render_ui( $post_id, $atts = [] ) {
		$formats = isset( $atts[ 'formats' ] ) ? array_map( 'trim', explode( ',', strtolower( $atts[ 'formats' ] ) ) ) : self::enabled_formats();
		$formats = array_values( array_intersect( [ 'pdf', 'epub' ], $formats ) );
		if ( empty( $formats ) )
			return '';
