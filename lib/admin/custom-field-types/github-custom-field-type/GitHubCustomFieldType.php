<?php
if ( ! class_exists( 'Read_Offline_GitHubCustomFieldType' ) ) :
/**
 * @version     1.0.1
 */
class Read_Offline_GitHubCustomFieldType extends Read_Offline_AdminPageFramework_FieldType {
    ...
    protected function getField( $aField ) { 
        ...
        return 
            $aField['before_label']
            . "<div class='read-offline-input-label-container'>"        
                . $aField['before_input']
                . ( $aField['label'] && ! $aField['repeatable']
                    ? "<span class='read-offline-input-label-string' style='min-width:" . $this->getLengthSanitized( $aField['label_min_width'] ) . ";'>" . $aField['label'] . "</span>"
                    : "" 
                )
        ...
        private function _getButtonLabelByType( $sButtonType, $sUserName, $sValue ) {
            ...
            switch( $sButtonType ) {
                case 'follow': 
                    return sprintf( __( 'Follow @%1$s', 'read-offline' ), $sUserName );
                case 'watch':
                    return __( 'Watch', 'read-offline' );
                case 'star':
                    return __( 'Star', 'read-offline' );
                case 'fork':
                    return __( 'Fork', 'read-offline' );
                case 'issue':
                    return __( 'Issue', 'read-offline' );
                default:
                    return '';
            }                 
