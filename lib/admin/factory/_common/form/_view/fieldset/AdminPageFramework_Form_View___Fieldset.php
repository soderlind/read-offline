...
abstract class Read_Offline_Settings_AdminPageFramework_Form_View___Fieldset_Base extends Read_Offline_Settings_AdminPageFramework_Form_Utility {
    ...
    private function _getFormatted($aFieldset, $aFieldTypeDefinitions) {
        return $this->uniteArrays($aFieldset, $this->_getFieldTypeDefaultArguments($aFieldset['type'], $aFieldTypeDefinitions) + Read_Offline_Settings_AdminPageFramework_Form_Model___Format_Fieldset::$aStructure);
    }
    ...
    private function _loadScripts($sStructureType = '') {
        if ('widget' === $sStructureType && !self::$_bIsLoadedJSScripts_Widget) {
            new Read_Offline_Settings_AdminPageFramework_Form_View___Script_Widget;
        ...
        new Read_Offline_Settings_AdminPageFramework_Form_View___Script_Utility;
        new Read_Offline_Settings_AdminPageFramework_Form_View___Script_OptionStorage;
        new Read_Offline_Settings_AdminPageFramework_Form_View___Script_AttributeUpdator;
        new Read_Offline_Settings_AdminPageFramework_Form_View___Script_RepeatableField($this->oMsg);
        new Read_Offline_Settings_AdminPageFramework_Form_View___Script_SortableField;
    }
...
jQuery( document ).ready( function() {
    var _oButtonPlaceHolders = jQuery( '#{$sFieldsContainerID} > .read-offline-field.without-child-fields .repeatable-field-buttons' );
    ...
    /* Otherwise, insert the button element at the beginning of the field tag */
         ...
         */
        if ( ! jQuery( '#{$sFieldsContainerID} .read-offline-repeatable-field-buttons' ).length ) { 
            jQuery( '#{$sFieldsContainerID} > .read-offline-field.without-nested-fields' ).prepend( $_sSmallButtons );
        }
        /**
         * Support for nested fields.
         * For nested fields, add the buttons to the fields tag.
         */
        jQuery( '#{$sFieldsContainerID} > .read-offline-field.with-nested-fields' ).prepend( $_sNestedFieldsButtons );
        
        /**
         * Support for inline mixed fields.
         */
        // jQuery( '#{$sFieldsContainerID} > .read-offline-field.with-mixed-fields' ).prepend( $_sNestedFieldsButtons );
        
    }     
    jQuery( '#{$sFieldsContainerID}' ).updateRead_Offline_Settings_AdminPageFrameworkRepeatableFields( $_aJSArray ); // Update the fields     
});
JAVASCRIPTS;
    ...
    private function ___getRepeatableButtonHTML($sFieldsContainerID, array $aArguments, $iFieldCount, $bSmall = true) {
        $_oFormatter = new Read_Offline_Settings_AdminPageFramework_Form_Model___Format_RepeatableField($aArguments, $this->oMsg);
    ...
    private function ___getContainerAttributes($aArguments) {
        $_aContainerAttributes = array('class' => $this->getClassAttribute('read-offline-repeatable-field-buttons', !empty($aArguments['disabled']) ? 'disabled' : ''),);
    ...
    protected function _getSortableFieldEnablerScript($sFieldsContainerID) {
        $_sScript = <<<JAVASCRIPTS
    jQuery( document ).ready( function() {
        jQuery( this ).enableRead_Offline_Settings_AdminPageFrameworkSortableFields( '$sFieldsContainerID' );
    });
JAVASCRIPTS;
        return "<script type='text/javascript' class='read-offline-sortable-field-enabler-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
}
class Read_Offline_Settings_AdminPageFramework_Form_View___Fieldset extends Read_Offline_Settings_AdminPageFramework_Form_View___Fieldset_Base {
    public function get() {
        $_aOutputs = array();
        $_oFieldError = new Read_Offline_Settings_AdminPageFramework_Form_View___Fieldset___FieldError($this->aErrors, $this->aFieldset['_section_path_array'], $this->aFieldset['_field_path_array'], $this->aFieldset['error_message']);
        $_aOutputs[] = $_oFieldError->get();
        $_oFieldsFormatter = new Read_Offline_Settings_AdminPageFramework_Form_Model___Format_Fields($this->aFieldset, $this->aOptions);
    ...
    private function _getEachFieldOutput($aField, $isIndex, array $aCallbacks, $bIsLastElement = false) {
        ...
        $_oSubFieldFormatter = new Read_Offline_Settings_AdminPageFramework_Form_Model___Format_EachField($aField, $isIndex, $aCallbacks, $_aFieldTypeDefinition);
    ...
    private function _getFieldOutput($sContent, $aField, $bIsLastElement) {
        $_oFieldAttribute = new Read_Offline_Settings_AdminPageFramework_Form_View___Attribute_Field($aField);
    ...
    private function _getFinalOutput($aFieldset, array $aFieldsOutput, $iFieldsCount) {
        $_oFieldsetAttributes = new Read_Offline_Settings_AdminPageFramework_Form_View___Attribute_Fieldset($aFieldset);
    ...
    private function _getEmbeddedFieldTitle($aFieldset) {
        ...
        $_oFieldTitle = new Read_Offline_Settings_AdminPageFramework_Form_View___FieldTitle($aFieldset, '', $this->aOptions, $this->aErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
    ...
    private function _getChildFieldTitle($aFieldset) {
        ...
        $_oFieldTitle = new Read_Offline_Settings_AdminPageFramework_Form_View___FieldTitle($aFieldset, array('read-offline-child-field-title'), $this->aOptions, $this->aErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
        return $_oFieldTitle->get();
    }
    private function _getFieldsetContent($aFieldset, $aFieldsOutput, $iFieldsCount) {
        ...
        $_oFieldsAttributes = new Read_Offline_Settings_AdminPageFramework_Form_View___Attribute_Fields($aFieldset, array(), $iFieldsCount);
    ...
    private function _getExtras($aField, $iFieldsCount) {
        $_aOutput = array();
        $_oFieldDescription = new Read_Offline_Settings_AdminPageFramework_Form_View___Description($aField['description'], 'read-offline-fields-description');
