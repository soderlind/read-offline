...
class Read_Offline_AdminPageFramework_Form_View___Script_RepeatableField extends Read_Offline_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updateRead_Offline_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.read-offline-repeatable-field-buttons' )
            .filter( function() {                                       
                return $( this ).closest( '.read-offline-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            });       
...
// @todo event.preventDefault();        
            ...
            $( this ).addRead_Offline_AdminPageFrameworkRepeatableField();
...
// @todo event.preventDefault();        
            $( this ).removeRead_Offline_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.read-offline-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.read-offline-field' )
            .filter( function() {                 
                return $( this ).closest( '.read-offline-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            })        
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addRead_Offline_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addRead_Offline_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        ...
        if ( ! _aOptions ) {     
            var _nodeButtonContainer = nodeFieldContainer.find( '.read-offline-repeatable-field-buttons' )
                .filter( function() {                 
                    return $( this ).closest( '.read-offline-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.read-offline-field' ) 
                                    .filter( function() {                 
                                        return $( this ).closest( '.read-offline-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.read-offline-repeatable-field-buttons' )
                                    .filter( function() {                 
                                        return $( this ).closest( '.read-offline-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    }); 
         ...
         */
        nodeNewField.updateRead_Offline_AdminPageFrameworkRepeatableFields();
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {                      
                return $( this ).closest( '.read-offline-fields' ).attr( 'id' ) === _sFieldsContainerID;                
            })
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeRead_Offline_AdminPageFrameworkRepeatableField = function() {
