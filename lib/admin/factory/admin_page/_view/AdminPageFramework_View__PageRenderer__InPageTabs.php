...
class Read_Offline_Settings_AdminPageFramework_View__PageRenderer__InPageTabs extends Read_Offline_Settings_AdminPageFramework_FrameworkUtility {
    public $oFactory;
    public $sPageSlug;
    public $sTag = 'h3';
    ...
    private function _getOutput($aInPageTabs, $sCurrentPageSlug, $sTag) {
        ...
        if (!$_aPage['show_in_page_tabs']) {
            return $this->getElement($aInPageTabs, array($_sCurrentTabSlug, 'title')) ? "<{$_sTag} class='read-offline-in-page-tab-title'>" . $aInPageTabs[$_sCurrentTabSlug]['title'] . "</{$_sTag}>" : "";
        }
    ...
    private function _getInPageTabNavigationBar(array $aTabs, $sActiveTab, $sCurrentPageSlug, $sTag) {
        $_oTabBar = new Read_Offline_Settings_AdminPageFramework_TabNavigationBar($aTabs, $sActiveTab, $sTag, array('class' => 'in-page-tab',), array('format' => array($this, '_replyToFormatNavigationTabItem_InPageTab'), 'arguments' => array('page_slug' => $sCurrentPageSlug,),));
        $_sTabBar = $_oTabBar->get();
        return $_sTabBar ? "<div class='read-offline-in-page-tab'>" . $_sTabBar . "</div>" : '';
    }
    public function _replyToFormatNavigationTabItem_InPageTab(array $aTab, array $aStructure, array $aTabs, array $aArguments = array()) {
        $_oFormatter = new Read_Offline_Settings_AdminPageFramework_Format_NavigationTab_InPageTab($aTab, $aStructure, $aTabs, $aArguments, $this->oFactory);
